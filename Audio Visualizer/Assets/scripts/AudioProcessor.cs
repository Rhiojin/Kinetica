...
public class AudioProcessor : MonoBehaviour
{
	...
	float onset, specVal, dbInc, aMax, acVal, score, smin, smax;
	int tempopd, smaxix;

	private List<AudioCallbacks> callbacks = new List<AudioCallbacks>();

	public delegate void onBeatDetectedDelegate();
	public event onBeatDetectedDelegate onBeatDetected;

	public delegate void onSpectrumDetectedDelegate(float[] averages);
	public event onSpectrumDetectedDelegate onSpectrumDetected;
...
//            }
			...
			if(onSpectrumDetected != null)
			{
				onSpectrumDetected(averages);
			}
            ...
            {
                specVal = (float)System.Math.Max(-100.0f, 20.0f * (float)System.Math.Log10(averages[i]) + 160); // dB value of this band
                specVal *= 0.025f;
                dbInc = specVal - spec[i]; // dB increment since last frame
            ...
            {
                acVal = (float)System.Math.Sqrt(auco.autoco(i));
                if (acVal > aMax)
                {
                    aMax = acVal;
                    tempopd = i;
            ...
            for (int i = tempopd / 2; i < System.Math.Min(colmax, 2 * tempopd); ++i)
            {
                // objective function - this beat's cost + score to last beat + transition penalty
               	score = onset + scorefun[(now - i + colmax) % colmax] - alph * (float)System.Math.Pow(System.Math.Log((float)i / (float)tempopd), 2);
